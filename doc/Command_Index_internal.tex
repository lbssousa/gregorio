...
\macroname{\textbackslash gre@boxedscore@build}{\#1}{gregoriotex-main.tex}
The common engine of \verb=\boxedgregorioscore=.  It sets \verb=\hsize= (to
\verb=\gre@dimen@boxedscorewidth=, or to \verb=\linewidth= if
\verb=\ifgre@boxedscorewidth@auto= is true) inside a
\verb=\vtop=, then runs \#1 inside it.  Since \verb=\GreBeginScore= and
\verb=\GreEndScore= make no assumption about \verb=\hsize= beyond using it,
this is enough to confine an ordinary score paragraph, initial letter and
all, to the given width.

\begin{argtable}
  \#1 & code & Code which typesets the score itself, e.g.\
  \verb=\gregorioscore{name}=.\\
\end{argtable}

\macroname{\textbackslash gre@boxedgregorioscore}{\#1}{gregoriotex-main.tex}
Macro that handles \verb=\boxedgregorioscore= calls when they do not have an
optional argument.
...
\macroname{\textbackslash gre@boxedgregorioscore@option}{[\#1]\#2}{gregoriotex-main.tex}
Macro that handles \verb=\boxedgregorioscore= calls when they have an
optional argument.  Behaves exactly like \verb=\gre@gregorioscore@option=,
forwarding \#1 and \#2 to \verb=\gregorioscore= inside
\verb=\gre@boxedscore@build=.

\begin{argtable}
  \#1 & \texttt{n}/\texttt{a}/\texttt{f} & Same meaning as for
  \verb=\gregorioscore=.\\
  \#2 & string & Relative or absolute path to the score.\\
\end{argtable}

\macroname{\textbackslash gre@snippet@build}{\#1\#2\#3}{gregoriotex-snippet.tex}
The common engine of the inline snippet macros.  It typesets the snippet's
score in an \verb=\hbox=, i.e.\ in restricted horizontal mode: the score is
then not a paragraph and never reaches the line breaker, so the box comes out
at the natural width of the music, which is what an inline snippet wants, and
a forced line break (\texttt{z} or \texttt{Z}) in the gabc has no effect.
Because neither of the line-break callbacks runs on such a score,
\verb=gregoriotex.finish_snippet= applies to the box what a score line gets
from them: the syllable spacing, the vertical adjustment of nabc, lyrics and
translations, and the setting of the staff lines to the width of the music.
That can only happen once the box is closed, by which point \TeX\ has restored
the settings the score made inside it, so the last thing inside the box is a
call to \verb=gregoriotex.capture_score_state=, which takes a copy of the
state the vertical adjustment depends on while the score is still open.

\begin{argtable}
  \#1 & string & Extra gabc headers, or empty.\\
  \#2 & code & Additional settings applied inside the snippet group.\\
  \#3 & string & The gabc body of the snippet.\\
\end{argtable}

\macroname{\textbackslash gre@snippet@hook}{}{gregoriotex-snippet.tex}
The user hook set by \verb=\gresetinlinesnippethook=, run inside the snippet
group just before the score itself.

\macroname{\textbackslash gre@snippet@savestate}{}{gregoriotex-snippet.tex}
Records the settings which a snippet may change globally (the staff size, and
whether the staff dimensions are zeroed), so that
\verb=\gre@snippet@restorestate= can put them back.  A \TeX\ group is not
enough, since those settings are set with \verb=\global=.  The staff size is
changed by the user's \verb=\gresetinlinesnippethook=, the zeroing by
\verb=\gre@neumesnippet@settings=.

\macroname{\textbackslash gre@snippet@restorestate}{}{gregoriotex-snippet.tex}
Restores what \verb=\gre@snippet@savestate= recorded and recomputes the
derived distances.

\macroname{\textbackslash gre@snippet@restorezero}{}{gregoriotex-snippet.tex}
Set by \verb=\gre@snippet@savestate= to \verb=\gre@staffdimensions@dozero= or
\verb=\gre@staffdimensions@unzero=, whichever puts back the staff dimension
zeroing in force before the snippet.

\macroname{\textbackslash gre@inlinesnippet@option}{[\#1]\#2}{gregoriotex-snippet.tex}
Internal to \verb=\gabcinlinesnippet=.

\macroname{\textbackslash gre@neumesnippet@option}{[\#1]\#2}{gregoriotex-snippet.tex}
Internal to \verb=\gabcneumesnippet=.

\macroname{\textbackslash gre@neumesnippet@settings}{}{gregoriotex-snippet.tex}
The settings which turn a snippet into a bare neume: invisible staff lines,
invisible clef, and no space reserved for the missing clef.  Making the staff
lines invisible zeroes the staff dimensions, which collapses the score onto
the text baseline without affecting the shape of the neume, since the pitch
offsets computed by \verb=\gre@calculate@glyphraisevalue= derive from
\verb=\gre@dimen@interstafflinedistancebase= rather than from
\verb=\gre@dimen@interstafflinespace=.
...
\macroname{\textbackslash gre@stackedlyric@endpart}{}{gregoriotex-main.tex}
Macro which stores the end part of the additional lyric line being typeset by \verb=\GreWriteStackedLyric= (that which comes after the alignment part).

\macroname{\textbackslash gre@stackedlyric@firstpart}{}{gregoriotex-main.tex}
Macro which stores the first part of the additional lyric line being typeset by \verb=\GreWriteStackedLyric= (that which comes before the alignment part).

\macroname{\textbackslash gre@stackedlyric@middlepart}{}{gregoriotex-main.tex}
Macro which stores the middle part of the additional lyric line being typeset by \verb=\GreWriteStackedLyric= (the part which aligns with the notes).
...
\macroname{\textbackslash gre@nabc@printchar}{\#1\#2\#3}{gregoriotex-nabc.tex}
Common implementation of \verb|\gre@nabcvoicechar| and
\verb|\gre@nabcneumesnippet|: selects the font of a nabc voice and prints a
nabc string with it.  The above-lines and below-lines styles are meant for
positioning inside a score, so \verb|\nabcneumesnippet| asks for them to be
skipped.

\begin{argtable}
  \#1 & integer & the voice whose above/below-lines style is applied, or \texttt{0} for none\\
  \#2 & integer & the voice whose font is used\\
  \#3 & string & The \texttt{nabc} syntax which indicates what neumes are to be printed.\\
\end{argtable}

\macroname{\textbackslash gre@nabcneumesnippet}{[\#1]\{\#2\}}{gregoriotex-nabc.tex}
Internal to \verb|\nabcneumesnippet|.
...
Internal to \verb|\gresetnabcfont|: selects the font of a nabc voice globally.
...
\macroname{\textbackslash gre@nabcvoicefont@local}{[\optional{\#1}]\{\#2\#3\}}{gregoriotex-nabc.tex}
Internal to \verb|\grenabcfont|: same as \verb|\gre@setnabcvoicefont|, but the
selection is local to the current \TeX\ group.

\begin{argtable}
  \#1 & integer & Optional. The nabc voice: 1 (default) = above staff, 2 = below staff.\\
  \#2 & string & the name of the font\\
  \#3 & integer & point size at which the font should be loaded\\
\end{argtable}

\macroname{\textbackslash gre@nabcvoicefont@do}{\#1\#2\#3\#4}{gregoriotex-nabc.tex}
Common implementation of \verb|\gre@setnabcvoicefont| and
\verb|\gre@nabcvoicefont@local|.

\begin{argtable}
  \#1 & code & \verb|\global| for a global selection, empty for a local one\\
  \#2 & integer & the nabc voice\\
  \#3 & string & the name of the font\\
  \#4 & integer & point size at which the font should be loaded\\
\end{argtable}

\macroname{\textbackslash gre@nabc@ensurefont}{\#1}{gregoriotex-nabc.tex}
Loads the default font of a nabc voice if no font has been selected for it yet.

\begin{argtable}
  \#1 & integer & the nabc voice\\
\end{argtable}
...
Prints the nabc glyphs for the given nabc string.  The scaling factor applied
to the font metrics is derived from the size of the currently selected font.

\begin{argtable}
  \#1 & string & nabc code representing the character\\
  \#2 & string & name of nabc font to use\\
  \#3 & integer & voice number (1 = above staff, 2 = below staff)\\
\end{argtable}
...
\macroname{\textbackslash ifgre@boxedscorewidth@auto}{}{gregoriotex-main.tex}
Boolean used to indicate whether \verb=\boxedgregorioscore= should use
\verb=\linewidth= (true, the default) or the width set by
\verb=\gresetboxedscorewidth= (false).  Set by \verb=\gresetboxedscorewidth=
and \verb=\gresetboxedscorewidthauto=.
...
\macroname{\textbackslash gre@dimen@stackedlyriccenter}{}{gregoriotex-main.tex}
Dimension used by \verb=\GreWriteStackedLyric= to hold the alignment center of the additional lyric line being typeset (the distance from the beginning of its letters to the middle of its middle part).

\macroname{\textbackslash gre@dimen@stackedlyricraise}{}{gregoriotex-main.tex}
Dimension used by \verb=\GreWriteStackedLyric= to hold the raise applied to the additional lyric line being typeset: \texttt{spacebeneathtext} minus (level $-$ 1) times \texttt{lyricstackseparation}.
...
\macroname{\textbackslash gre@box@snippet}{}{gregoriotex-snippet.tex}
Box in which an inline snippet is built.  It first holds the score as \TeX\
packed it, and is then replaced by the finished snippet, repacked at its
natural width by \verb=gregoriotex.finish_snippet=.
...
\macroname{\textbackslash gre@dimen@snippetraise}{}{gregoriotex-snippet.tex}
The vertical offset applied to inline snippets, set by
\verb=\gresetinlinesnippetraise=.

\macroname{\textbackslash gre@dimen@boxedscorewidth}{}{gregoriotex-main.tex}
The width at which \verb=\boxedgregorioscore= typesets its score, set by
\verb=\gresetboxedscorewidth=.  Only used when \verb=\ifgre@boxedscorewidth@auto=
is false.
