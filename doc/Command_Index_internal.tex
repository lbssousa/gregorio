...
Macro called before each glyph.  When \verb=\ifgre@nabc@interglyphpadding= is true, performs a two-phase per-voice right-balance check.  Phase~1: saves the previous glyph's combined and per-voice right-balances (\verb=\gre@dimen@nabc@rightbalance=, \verb=@i=, \verb=@ii=) into their \verb=prevbalance= counterparts, then resets the \verb=rightbalance= registers to 0pt.  Phase~2 (after the NABC pre-phase runs): evaluates each voice independently --- a voice contributes to the kern accumulator only if \emph{both} its \verb=prevbalance@\textit{v}= is positive (previous glyph overflowed in that voice) \emph{and} its \verb=rightbalance@\textit{v}= is positive (NABC content was armed for this glyph in that voice).  Voices that overflowed but whose next snippet is empty have their \verb=rightbalance@\textit{v}= restored so the overflow continues to be tracked and reduced by subsequent horizontal advances.  The combined \verb=rightbalance= is recomputed as \(\max(\verb=@i=, \verb=@ii=)\) after any restores.  The final kern is the maximum contribution across all voices.  This prevents spurious padding through \texttt{/} glyph breaks, \texttt{@} neume fusions, and empty voice snippets.  Also handles the NABC two-phase pre/emit processing for all voices.
...
Macro for populating \verb=\gre@box@syllablenotes=.  Resets all NABC tracking
registers (\verb=\gre@dimen@nabc@maxwidth=, \verb=\gre@dimen@nabc@extrakern=,
combined and per-voice \verb=rightbalance=, \verb=prevbalance= including
\verb=@i= and \verb=@ii=) before the boxing pass, and resets the
\verb=rightbalance= and \verb=prevbalance= registers again after boxing to
prevent state leakage into the real rendering pass.  When notes are visible,
calls \verb=\gre@compute@nabc@extrakern= to widen the box if NABC overflows.
...
\macroname{\textbackslash gre@nabc@prepare}{\#1\#2\#3}{gregoriotex-main.tex}
Phase 1 of NABC two-phase processing: pre-evaluates NABC content, tracks the maximum NABC width in \verb=\gre@dimen@nabc@maxwidth= for right-overflow detection by \verb=\gre@compute@nabc@extrakern=, emits note-level kern for left overflow (in \texttt{neume} alignment mode) unless \verb=\ifgre@nabc@suppress@leftoverflow= is true, and saves the rendered content in a box register.  When \verb=\ifgre@nabc@interglyphpadding= is true, also sets the per-voice right-balance register (\verb=#3=) to the width of the rendered box so the per-voice kern decision in \verb=\gre@newglyphcommon= knows which voice overflowed.  The combined \verb=\gre@dimen@nabc@rightbalance= is no longer maintained here; it is recomputed as \(\max(\verb=@i=, \verb=@ii=)\) unconditionally in phase~2 of \verb=\gre@newglyphcommon=.

\begin{argtable}
  \#1 & content & The NABC content (from \textbackslash GreNABCChar)\\
  \#2 & box register & Box register to save the rendered content to\\
  \#3 & dimen register & Per-voice right-balance register to set (e.g.\ \verb=\gre@dimen@nabc@rightbalance@i=)\\
\end{argtable}
...
\macroname{\textbackslash gre@nabc@reduce@rightbalance}{\#1}{gregoriotex-nabc.tex}
Helper that reduces all three right-balance registers (\verb=\gre@dimen@nabc@rightbalance=, \verb=@i=, \verb=@ii=) by the horizontal advance \verb=#1=, guarded by \verb=\ifgre@nabc@interglyphpadding=.  Called from \verb=\GreGlyph= (glyph width), \verb=\GreEndOfGlyph= (inter-glyph spacing), \verb=\GreEndOfElement= (inter-element spacing), and \verb=\gre@alteration= (alteration width and spacing).

\begin{argtable}
  \#1 & dimen/skip & The amount to subtract from each right-balance register\\
\end{argtable}
...
\macroname{\textbackslash gre@dimen@nabc@rightbalance}{}{gregoriotex-nabc.tex}
Tracks the remaining combined NABC right-overflow within a syllable.  Recomputed as \(\max(\verb=@i=, \verb=@ii=)\) unconditionally in phase~2 of \verb=\gre@newglyphcommon=, then reduced by each horizontal advance via \verb=\gre@nabc@reduce@rightbalance=: glyph width (in \verb=\GreGlyph=), inter-glyph spacing (in \verb=\GreEndOfGlyph=), inter-element spacing (in \verb=\GreEndOfElement=), and alteration widths/spacing (in \verb=\gre@alteration=).  In \verb=\gre@newglyphcommon=, the value is saved into \verb=\gre@dimen@nabc@prevbalance= and then reset to 0pt before the NABC pre-phase runs.  Only active when \verb=\ifgre@nabc@interglyphpadding= is true.  Reset to 0pt at the start of each syllable and after the boxing pass.

\macroname{\textbackslash gre@dimen@nabc@rightbalance@i}{}{gregoriotex-nabc.tex}
Per-voice~1 (above staff) variant of \verb=\gre@dimen@nabc@rightbalance=.  Set inside \verb=\gre@nabc@prepare= via its third argument when called from the arm macros (\verb=\GreSetNabcAboveLines=, \verb=\GreSetNabcAboveLinesDeferred=).  Reduced by the same horizontal advances as the combined register (through \verb=\gre@nabc@reduce@rightbalance=).  Used in the phase~2 per-voice kern decision: if \verb=rightbalance@i > 0= after the pre-phase, it signals that voice~1 has new NABC content for the current glyph.

\macroname{\textbackslash gre@dimen@nabc@rightbalance@ii}{}{gregoriotex-nabc.tex}
Per-voice~2 (below staff) variant.  Mirrors \verb=\gre@dimen@nabc@rightbalance@i= for voice~2.  Set inside \verb=\gre@nabc@prepare= when called from \verb=\GreSetNabcBelowLines= / \verb=\GreSetNabcBelowLinesDeferred=.

\macroname{\textbackslash gre@dimen@nabc@prevbalance}{}{gregoriotex-nabc.tex}
Scratch register used inside \verb=\gre@newglyphcommon= to snapshot the combined right-balance before the NABC pre-phase resets it.  After the per-voice kern decision, it is reused as the kern accumulator (reset to 0pt, then set to the maximum contributing voice's \verb=prevbalance@\textit{v}=).

\macroname{\textbackslash gre@dimen@nabc@prevbalance@i}{}{gregoriotex-nabc.tex}
Per-voice~1 snapshot of \verb=\gre@dimen@nabc@rightbalance@i= taken in phase~1 of \verb=\gre@newglyphcommon=.  In phase~2, a voice contributes to the kern only if \emph{both} \verb=prevbalance@i > 0= (overflow) and \verb=rightbalance@i > 0= (new content armed).  If the voice overflowed but has no new content, \verb=rightbalance@i= is restored from this snapshot so the overflow continues to be tracked.

\macroname{\textbackslash gre@dimen@nabc@prevbalance@ii}{}{gregoriotex-nabc.tex}
Per-voice~2 snapshot.  Mirrors \verb=\gre@dimen@nabc@prevbalance@i= for voice~2.

\macroname{\textbackslash ifgre@nabc@interglyphpadding}{}{gregoriotex-nabc.tex}
Boolean flag controlling the inter-glyph NABC padding feature.  When true, \verb=\gre@newglyphcommon= performs a two-phase per-voice check: it saves the previous glyph's combined and per-voice right-balances into their \verb=prevbalance= counterparts, resets all \verb=rightbalance= registers to 0pt, lets the NABC pre-phase run (which re-arms only the voices that have new content), and then evaluates each voice independently.  A voice contributes to the kern accumulator only if both its \verb=prevbalance@\textit{v}= and \verb=rightbalance@\textit{v}= are positive.  Voices that overflowed but have empty next snippets have their \verb=rightbalance@\textit{v}= restored so the overflow continues to be tracked.  The final kern is the maximum across contributing voices.  The boxing pass reset of all \verb=rightbalance= registers to 0pt after \verb=\gre@syllablenotes= prevents state leakage between the boxing and real rendering passes.  Set via \verb=\gresetnabcinterglyphpadding{true|false}=.
