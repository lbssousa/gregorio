...
combined and per-voice \verb=rightbalance=, \verb=prevbalance=) before the
boxing pass, and resets the \verb=rightbalance= registers again after boxing to
...
Phase 1 of NABC two-phase processing: pre-evaluates NABC content, tracks the maximum NABC width in \verb=\gre@dimen@nabc@maxwidth= for right-overflow detection by \verb=\gre@compute@nabc@extrakern=, emits note-level kern for left overflow (in \texttt{neume} alignment mode) unless \verb=\ifgre@nabc@suppress@leftoverflow= is true, and saves the rendered content in a box register.  When \verb=\ifgre@nabc@interglyphpadding= is true, also updates the combined \verb=\gre@dimen@nabc@rightbalance= to the maximum NABC width across all voices for the current glyph.  Note: the per-voice \verb=rightbalance@i= / \verb=@ii= registers are set by the arm macros (\verb=\GreSetNabcAboveLines= etc.) \emph{after} calling \verb=\gre@nabc@prepare=, not inside this macro itself.
...
Tracks the remaining combined NABC right-overflow within a syllable.  Set to the maximum NABC width across all voices in \verb=\gre@nabc@prepare=, then reduced by each horizontal advance: glyph width (in \verb=\GreGlyph=), inter-glyph spacing (in \verb=\GreEndOfGlyph=), and inter-element spacing (in \verb=\GreEndOfElement=).  In \verb=\gre@newglyphcommon=, the value is saved into \verb=\gre@dimen@nabc@prevbalance= and then reset to 0pt before the NABC pre-phase runs.  After the per-voice kern decision, the combined value is recomputed as \(\max(\verb=@i=, \verb=@ii=)\).  Only active when \verb=\ifgre@nabc@interglyphpadding= is true.  Reset to 0pt at the start of each syllable and after the boxing pass.

\macroname{\textbackslash gre@dimen@nabc@rightbalance@i}{}{gregoriotex-nabc.tex}
Per-voice~1 (above staff) variant of \verb=\gre@dimen@nabc@rightbalance=.  Set by the arm macros (\verb=\GreSetNabcAboveLines=, \verb=\GreSetNabcAboveLinesDeferred=) to the width of the voice~1 NABC box.  Reduced by the same horizontal advances as the combined register.  Used in the phase~2 per-voice kern decision: if \verb=rightbalance@i > 0= after the pre-phase, it signals that voice~1 has new NABC content for the current glyph.

\macroname{\textbackslash gre@dimen@nabc@rightbalance@ii}{}{gregoriotex-nabc.tex}
Per-voice~2 (below staff) variant.  Mirrors \verb=\gre@dimen@nabc@rightbalance@i= for voice~2.  Set by \verb=\GreSetNabcBelowLines= / \verb=\GreSetNabcBelowLinesDeferred=.
