...
ordinary score paragraph inside a \verb=\vbox= whose \verb=\hsize= is
\verb=\gre@dimen@snippethsize=, so that the paragraph cannot break and
...
\macroname{\textbackslash gre@snippet@headers}{}{gregoriotex-snippet.tex}
Holds the gabc headers built by \verb=\gre@neumesnippet@option= before they
are passed to \verb=\gre@snippet@build=.
...
\macroname{\textbackslash gre@nabcsnippet}{[\#1]\{\#2\}}{gregoriotex-nabc.tex}
Internal to \verb|\nabcsnippet|.
...
\begin{argtable}
  ...
  \#3 & integer & scaling factor, or \texttt{0} to derive it from the size of the currently selected font\\
...
\macroname{\textbackslash gre@dimen@snippethsize}{}{gregoriotex-snippet.tex}
The \verb=\hsize= used while an inline snippet is typeset.  It must be wide
enough that no snippet ever breaks into several lines, but small enough that
dimension arithmetic cannot overflow, hence roughly \verb=\maxdimen= divided
by two.
