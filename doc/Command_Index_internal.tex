...
The common engine of the inline snippet macros.  It typesets the snippet as an
ordinary score paragraph inside a \verb=\vbox= whose \verb=\hsize= is huge
(roughly \verb=\maxdimen= divided by two: wide enough that the paragraph cannot
break, small enough that dimension arithmetic cannot overflow), so that
\verb=\parfillskip= absorbs all the slack; the \texttt{trimmed} last line mode
then makes the Lua layer set the staff lines to the natural width of the
score.  \verb=gregoriotex.finish_inline_snippet= extracts the single line of
that paragraph and repacks it as an \verb=\hbox= of natural width, which is
finally placed in the enclosing paragraph.
...
\macroname{\textbackslash gre@snippet@settings}{}{gregoriotex-snippet.tex}
The paragraph settings applied inside the \verb=\vbox= in which a snippet is
typeset.
...
Records the settings which a snippet may change globally (the end-of-line
custos, the staff size and whether the staff dimensions are zeroed), so that
\verb=\gre@snippet@restorestate= can put them back.  A \TeX\ group is not
enough, since those settings are set with \verb=\global=.
...
derived distances with the outer \verb=\hsize= in force.
...
Box in which an inline snippet is built.  It first holds the \verb=\vbox= in
which the snippet is typeset as an ordinary score paragraph, and is then
replaced by the single line of that paragraph, repacked at its natural width
by \verb=gregoriotex.finish_inline_snippet=.
