...
Macro to end elements.  When \verb=\ifgre@nabc@interglyphpadding= is true, reduces \verb=\gre@dimen@nabc@rightbalance= and its per-voice variants (\verb=@i=, \verb=@ii=) by the inter-element spacing that was just emitted.
...
Macro to end a glyph without ending the element.  When \verb=\ifgre@nabc@interglyphpadding= is true, reduces \verb=\gre@dimen@nabc@rightbalance= and its per-voice variants (\verb=@i=, \verb=@ii=) by the inter-glyph spacing that was just emitted (but does not emit a kern, since sub-glyphs within the same element are naturally covered by the element's NABC annotation).
...
Macro to place argument containing Nabc neumes above the lines.  Arms the
two-phase NABC machinery for voice~1.  When
\verb=\ifgre@nabc@interglyphpadding= is true, the pre-phase also sets
\verb=\gre@dimen@nabc@rightbalance@i= to the NABC box width so that the
per-voice kern decision in \verb=\gre@newglyphcommon= can distinguish
which voice overflowed.
...
Macro to place argument containing Nabc neumes below the lines.  Arms the
two-phase NABC machinery for voice~2.  When
\verb=\ifgre@nabc@interglyphpadding= is true, the pre-phase also sets
\verb=\gre@dimen@nabc@rightbalance@ii= to the NABC box width.
...
pre-phase has run.  When \verb=\ifgre@nabc@interglyphpadding= is true, the
pre-phase also sets \verb=\gre@dimen@nabc@rightbalance@i=.
...
and the \verb=\gre@nabc@*@voice@ii= macros.  When
\verb=\ifgre@nabc@interglyphpadding= is true, the pre-phase also sets
\verb=\gre@dimen@nabc@rightbalance@ii=.
