...
\macroname{\textbackslash GreBeginScore}{\#1\#2\#3}{gregoriotex-main.tex}
...
\macroname{\textbackslash GreWriteLyricLine}{\#1\#2\#3\#4\#5\#6\#7\#8}{gregoriotex-main.tex}
Macro to typeset one additional lyric line (level 2 or below) of a stacked
syllable.  It is called inside the eighth argument of \verb=\GreSyllable=,
where the current point is the left edge of the main syllable text, and
centers the line's own alignment point under the main line's one.

\begin{argtable}
  \#1 & integer & The lyric line level (2 for the first additional line).\\
  \#2 & \texttt{0} & This level does not end a word at this syllable.\\
  & \texttt{1} & This level ends a word at this syllable.\\
  \#3 & \texttt{0} & No forced center in this line.\\
  & \texttt{1} & This line has a forced (\texttt{\{\}}) center.\\
  \#4 & string & The first letters of the line, before the alignment point.\\
  \#5 & string & The middle letters of the line (the alignment point).\\
  \#6 & string & The end letters of the line.\\
  \#7 & string & The very first letter (for first-letter centering).\\
  \#8 & string & All the letters after the first one.\\
\end{argtable}

\macroname{\textbackslash GreScoreLyricLines}{\#1}{gregoriotex-main.tex}
Macro which sets the total number of lyric lines of the score (1 when the
score has no stacked lyrics).

\begin{argtable}
  \#1 & integer & the number of lyric lines of the score.\\
\end{argtable}
