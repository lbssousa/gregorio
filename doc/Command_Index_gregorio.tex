...
Macro to end elements.  When \verb=\ifgre@nabc@interglyphpadding= is true, reduces \verb=\gre@dimen@nabc@rightbalance= by the inter-element spacing that was just emitted.
...
Macro to end a glyph without ending the element.  When \verb=\ifgre@nabc@interglyphpadding= is true, reduces \verb=\gre@dimen@nabc@rightbalance= by the inter-glyph spacing that was just emitted (but does not emit a kern, since sub-glyphs within the same element are naturally covered by the element's NABC annotation).
