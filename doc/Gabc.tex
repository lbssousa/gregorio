...
\begin{description}
  \item[above lines text] Text between \texttt{<alt>} and \texttt{</alt>} is
    ``above lines text'' and will be typeset above the staff lines.  By
    default it is horizontally aligned with the left edge of the note it is
    anchored to (or the document-wide default set by the command
    \verb=\gresetabovelinestextalignment{<mode>}=, where \texttt{<mode>} is
    \texttt{left}, \texttt{center}, or \texttt{right}).  \texttt{<alt:l>},
    \texttt{<alt:c>}, or \texttt{<alt:r>} may be used instead of
    \texttt{<alt>} to force left, center, or right alignment for that
    occurrence, regardless of the document default.  Note that when the
    anchor note is immediately followed by a bar line, center and right
    alignment currently fall back to using the width of the previous note
    instead of the bar.  Similarly, on a score's very first syllable,
    center and right alignment may be thrown off by the extra spacing
    reserved for a following drop initial (see
    \verb=\gresetinitiallines=), since that spacing is applied after the
    alignment has already been computed against the note's un-shifted
    position.
...
\nameref{translationtext} that appears below the lyric text.  The pipe
(\texttt{|}) separates \nameref{multiplelyriclines}; a literal pipe must be
escaped as \texttt{\$|}.  The tilde
...
\subsubsection{Multiple Lyric Lines}\label{multiplelyriclines}

A syllable may carry several stacked lines of lyrics, separated by pipes
(\texttt{|}) in the syllable text:

\begin{verbatim}
Ma|Ma(f)ter|ter(g) pu| cas(h)rís|tís(g)si|si(f)ma|ma(g)
\end{verbatim}

{
  \gresetinitiallines{0}%
  \gregorioscore[a]{multiplelyriclines1}
}

The text before the first pipe is the main lyric line; each subsequent pipe
starts a further line below it.  Each line is centered under the notes
independently, following the current \nameref{lyriccentering} mode
(including a forced center in curly braces, settable per line).
Corresponding lines of neighboring syllables share a baseline, spaced by the
\texttt{lyricstackseparation} distance; any translation text goes below the
whole stack.

\textbf{Note:} \nameref{translationtext} is not itself stackable: a
syllable has a single translation, shared by every lyric line in its
stack.

Word boundaries are tracked per line, using the same rule everywhere: a
space right before a line's text starts a new word on that line.  On the
main line, that space is the usual one between two syllables; on the other
lines, it is the space right after the pipe that starts the line.  A
stacked word also ends when the following syllable does not reach that
level of the stack at all, whether because it has no stack of its own or
because its stack does not go that deep.  A level that \emph{is} present
but left empty --- two consecutive pipes, as in \texttt{ca||ci(h)}, or a
trailing empty line at the end of a stack --- does \emph{not} end the word:
it resumes with the next syllable that actually carries text on that level.
For example:

\begin{verbatim}
Do|Pa(h)mi|(g)nus|ter(h)
\end{verbatim}

{
  \gresetinitiallines{0}%
  \gregorioscore[a]{multiplelyriclines2}
}

The second line reads \textit{Pater}, continuing across \texttt{mi|}'s empty
text since the pipe still reaches that level. When the following syllable's
stack does not reach that level at all, the word ends there instead:

\begin{verbatim}
Im|San(h)ma|cta(g)cu(h)la|Ma(g)ta|ter(h)
\end{verbatim}

{
  \gresetinitiallines{0}%
  \gregorioscore[a]{multiplelyriclines3}
}

Here \texttt{cu} has no second line at all, so the second line reads two
separate words, \textit{Sancta} and \textit{Mater}, instead of continuing as
one. The same rule closes a trailing empty level at the end of the score
without a spurious hyphen, and governs automatic hyphens at line breaks on
every level.

A hyphen can be forced on any level the same way it can on the main line: a
literal \texttt{-} right before the \texttt{|} that closes a level (or,
for a stack's last level, right before the notes' opening \texttt{(})
forces a hyphen there regardless of what the automatic, width-based
hyphenation would otherwise decide:
...
{
  \gresetinitiallines{0}%
  \gregorioscore[a]{multiplelyriclinesforcedhyphen}
}
...
positioning.  To do this, use \texttt{[alt:}\textit{text}\texttt{]}.  As with
the \texttt{<alt>} tag, this is left-aligned with the note by default (or
uses the document-wide default set by
\verb=\gresetabovelinestextalignment=); appending \texttt{;l}, \texttt{;c},
or \texttt{;r} right before the closing \texttt{]} forces left, center, or
right alignment for that occurrence, e.g.\ \texttt{[alt:}\textit{text}%
\texttt{;c]} for centering or \texttt{[alt:}\textit{text}\texttt{;r]} for
right alignment.  Since a trailing \texttt{;l}, \texttt{;c}, or \texttt{;r}
is given this special meaning, if \textit{text} itself needs to end that
way literally, escape the semicolon with \texttt{\$} (\eg\
\texttt{[alt:}\textit{text}\texttt{\$;l]}) to keep it from being
interpreted as an alignment marker.
