...
Word boundaries are determined per line.  The main line follows the usual
rule: a space after the parenthesized notes ends the word.  The lower lines
(second and following) of two \emph{consecutive} stacked syllables are
considered syllables of the same word by default, even when the main-line
word ends in between.  In the example above, \texttt{Al-le-lu-ia} is a
single word on the second line, while \texttt{San-ctus} and
\texttt{Do-mi-nus} are words on the main line.  To end a word on a lower
line explicitly, insert a space between the text and the following pipe or
opening parenthesis, for instance:

\begin{verbatim}
ba|be (f)da|de(g)
\end{verbatim}

Here \texttt{be} and \texttt{de} are separate words on the second line
(because of the space before \texttt{(f)}), while \texttt{ba} and
\texttt{da} still belong to the same word on the main line.  A stacked word
...
Here the second line reads \textit{veneração}: even though \texttt{ção} has
no trailing space, it still ends the word, because the last syllable's
second line is left empty --- so no automatic hyphen is generated after
\texttt{ção}.  Automatic hyphens follow these per-line word boundaries,
including at line breaks.
