...
\begin{verbatim}
San|Al(f)ctus|le(g)Do|lu(h)mi|ia(g)nus(f)
\end{verbatim}

The text before the first pipe is the main (first) lyric line; each
subsequent pipe starts one line further below the staff.  Each line of the
stack is centered under the notes independently, following the current
\nameref{lyriccentering} mode (including forced centering with curly
braces, which may be used separately in each line).  Corresponding lines of
neighboring syllables share a common baseline, and the vertical separation
between lines is controlled by the \texttt{lyricstackseparation} distance.
The translation text, if any, is placed below the whole stack.

Word boundaries are determined per line, by the same rule on every line: a
space before the text of a syllable starts a new word on that line.  On the
main line this is the familiar gabc rule, the space sitting between the
closing parenthesis of the previous syllable and the text of this one; on
the lines of a stack, the space goes right after the pipe that starts the
line.  Without such a space, the lower lines (second and following) of two
\emph{consecutive} stacked syllables are considered syllables of the same
word, even when the main-line word ends in between.  In the example above,
\texttt{Al-le-lu-ia} is a single word on the second line, while
\texttt{San-ctus} and \texttt{Do-mi-nus} are words on the main line.
Another example:

\begin{verbatim}
ca|ba(h)ra|la(h)me| bo(h)lo|lo(h)
\end{verbatim}

Here the main line is the single word \textit{caramelo}, while the second
line holds two words, \textit{bala} and \textit{bolo}: the space before
\texttt{bo} starts the second one.  A stacked word
also ends whenever the following syllable does not carry non-empty text on
the corresponding line --- because that syllable does not reach that deep
in the stack, because it is a plain syllable with no stack at all, or
because the line is explicitly left empty there, as in \texttt{ca||ci(h)}
(two consecutive pipes, skipping the second line for that syllable).  For
instance:

\begin{verbatim}
so|ve(h)le|ne(h)ni|ra(h)da|ção(h)de|(h)
\end{verbatim}

Here the second line reads \textit{veneração}: the word ends at
\texttt{ção}, because the last syllable's second line is left empty, so no
automatic hyphen is generated after it.  Automatic hyphens follow these
per-line word boundaries, including at line breaks.

Because the pipe starts a new lyric line, a literal \texttt{|} in syllable
text must be escaped as \texttt{\$|}.
