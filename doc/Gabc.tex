...
also ends when the following syllable with text does not carry the
corresponding line (for example, when a plain syllable follows a stack).
Automatic hyphens follow these per-line word boundaries, including at line
breaks.

A line of the stack may be left empty (as in \texttt{ca||ci(h)}) to skip a
level in that syllable.  Because the pipe starts a new lyric line, a
literal \texttt{|} in syllable text must be escaped as \texttt{\$|}.
