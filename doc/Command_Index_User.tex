...
instead of starting a new one.  The snippet must fit on a single line; the
...
If the snippet contains a forced line break (\texttt{z} or \texttt{Z}), only
the first line is kept and a warning is issued.
...
alignment specified in the gabc source (\texttt{<alt:l>}, \texttt{<alt:c>},
\texttt{<alt:r>}, or the \texttt{[alt:}\textit{text}\texttt{;l]},
\texttt{[alt:}\textit{text}\texttt{;c]}, \texttt{[alt:}\textit{text}%
\texttt{;r]} note-level forms) overrides this default.
...
size for a single \verb|\nabcsnippet| without disturbing the rest of the
...
\macroname{\textbackslash nabcsnippet}{[\optional{\#1}]\{\#2\}}{gregoriotex-nabc.tex}
Typesets ancient notation glyphs on their own, with no square notation and no
staff, at the point where the macro appears.
...
\medskip For example:\par\medskip
\begin{latexcode}
  the virga \nabcsnippet{vi} and the pes \nabcsnippet{pe}, and the same
  in Laon: {\grenabcfont[1]{grelaon}{10}\nabcsnippet{vi} \nabcsnippet{pe}}
\end{latexcode}
...
\begin{center}
  \scalebox{2}{%
    ...
    \end{mplibcode}%
    \hspace{0.2cm}%
    \lines{\nabcdisplay{2}{<alt>alt</alt>text[trans](g|vi|vi)}}%
  }
\end{center}
...
\ttfamily
\begin{tabular}{ccccc}
  \lines{\nabcdisplay{1}{m|vi[0 steps](m|vi)}} &
  \lines{\nabcdisplay{1}{m_1|vi[0 steps](m_1|vi)}} &
  \lines{\nabcdisplay{1}{m<v>\textquotesingle</v>1|vi[1 step](m'1|vi)}} &
  \lines{\nabcdisplay{1}{m_1<v>\textquotesingle</v>1|vi[2 steps](m_1'1|vi)}}
\end{tabular}
\end{center}
\end{quote}
