...
- Fixed vertical spacing so that rare signs (accentus, circulus, semicirculus, musica ficta) no longer collide with above-lines text, and so that below-lines nabc no longer collides with the low note it sits under; the new count `additionalbottomspacenabcthreshold` gives it the same reactive behavior that the lyrics already had, independently of the lyrics threshold. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
