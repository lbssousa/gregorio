...
### Added
- Support for multiple (stacked) lyric lines: `ba|be|bi(fg)` typesets `be` and `bi` on additional lyric lines below `ba`. Each line is centered under the notes independently and has its own word boundaries (the lower lines of consecutive stacks belong to the same word by default; a space at the beginning of a line starts a new word on it, which is the same rule the main line follows), with automatic hyphenation per line. The vertical separation between lines is controlled by the new `lyricstackseparation` distance. A system that uses stacking gets exactly the extra interline room its own stack needs above the *next* system, so the added depth does not collapse that one gap; other systems in the same score are unaffected. A literal `|` in syllable text must now be escaped as `$|`. See [#374](https://github.com/gregorio-project/gregorio/issues/374).
