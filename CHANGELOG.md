...
- Fixed vertical spacing so that rare signs (accentus, circulus, semicirculus, musica ficta) no longer collide with above-lines text, and so that below-lines nabc no longer collides with the low note it sits under; the new count `additionalbottomspacenabcthreshold` gives it the same reactive behavior that the lyrics already had, independently of the lyrics threshold. Also fixed two related gaps that could leave this space uncollapsed or unreserved: hiding staff lines alone (`\gresetlines{invisible}`) no longer collapses the pitch grid unless notes and the clef are hidden too (which was misplacing the notes, nabc, and above-lines text still drawn on it), and hiding notes while nabc stays visible (`\gresetnotes{invisible}`) no longer discards the pitch data — including rare signs — that this collision avoidance depends on. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
...
- `\gabcsnippet` passed the `--deprecation-errors` option to `gregorio` when deprecated constructs were *allowed* instead of when they were forbidden, i.e. exactly the other way round from `\gregorioscore`. See [#1771](https://github.com/gregorio-project/gregorio/issues/1771).
- The metrics of a nabc font were cached at the size at which the font was first loaded, so using the same nabc font at two different sizes in one document gave wrong raises and kerns for the second one. They are now scaled to the size of the font actually in use. See [#1771](https://github.com/gregorio-project/gregorio/issues/1771).
