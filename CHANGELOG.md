...
- Fixed vertical spacing around nabc lines to avoid several overlaps: rare signs (accentus, circulus, semicirculus, musica ficta) no longer overlap above-lines text; below-lines nabc no longer overlaps the low note it sits under, now reacting to its own threshold (new count `additionalbottomspacenabcthreshold`) independently of the lyrics threshold; and an nabc glyph shifted by an explicit `hX` pitch code no longer overlaps the staff or the text on the far side of its nabc line. Also fixed two related gaps that could leave this reserved space uncollapsed or missing entirely: hiding staff lines alone (`\gresetlines{invisible}`) no longer collapses the pitch grid unless notes and the clef are hidden too (which was misplacing the notes, nabc, and above-lines text still drawn on it), and hiding notes while nabc stays visible (`\gresetnotes{invisible}`) no longer discards the pitch data — including rare signs — that this overlap avoidance depends on. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
...
- Fixed two bugs affecting snippets and ancient notation. `\gabcsnippet` passed the `--deprecation-errors` option to `gregorio` when deprecated constructs were *allowed* instead of when they were forbidden, i.e. exactly the other way round from `\gregorioscore`. The metrics of a nabc font were cached at the size at which the font was first loaded, so using the same nabc font at two different sizes in one document gave wrong raises and kerns for the second one; they are now scaled to the size of the font actually in use. See [#1771](https://github.com/gregorio-project/gregorio/issues/1771).

### Added
- Added two ways to embed a gabc/gtex score other than as a full-width display. Inline gabc+nabc snippets set a small score inside a paragraph of running text: `\gabcinlinesnippet` for a fragment of score (staff lines trimmed to the width of the music), `\gabcneumesnippet` for a bare neume with no staff and no clef, optionally accompanied by nabc glyphs above and below, and `\nabcneumesnippet` for nabc glyphs on their own; `\grenabcfont` selects the nabc font and size for the current TeX group only, and `\gresetinlinesnippetraise`/`\gresetinlinesnippethook` tune how snippets are typeset. `\boxedgregorioscore` instead keeps a score as its own paragraph but confines it to a given width -- `\linewidth` by default, which is correct both in a table cell and in a list item, or an explicit width set with `\gresetboxedscorewidth` -- so, unlike an inline snippet, it may wrap onto several lines and keeps full support for initial letters, including ones spanning several lines. See [#1771](https://github.com/gregorio-project/gregorio/issues/1771).
...
- Above-lines text can now be centered or right-aligned relative to the note it's anchored to, instead of always sitting flush left. Force a one-off alignment with `<alt-l>`/`</alt-l>`, `<alt-c>`/`</alt-c>`, or `<alt-r>`/`</alt-r>`, or with `[alt-l:`*text*`]`, `[alt-c:`*text*`]`, or `[alt-r:`*text*`]`, or change the document-wide default with the new `\gresetabovelinestextalignment{left|center|right}`. See [#1768](https://github.com/gregorio-project/gregorio/issues/1768).
