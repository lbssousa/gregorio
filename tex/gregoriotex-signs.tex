  ...
  \ifgre@boxing\else %
    ...
    \or %
      ...
      \ifnum#1<\gre@pitch@raresign\relax %
        ...
        \GreGlyphHeights{\gre@pitch@raresign}{\gre@pitch@raresign}%
      \else %
        {%
          \gre@count@temp@three=\numexpr(#1 + 1)\relax %
          \GreGlyphHeights{\gre@count@temp@three}{\gre@count@temp@three}%
...
\def\gre@alteration@visible#1#2#3#4#5#6{%
  ...
  \setbox\gre@box@temp@width=\hbox
    ...
    \fi}%
  % Record the width before \gre@newglyphcommon (which may typeset pending
  % above-lines text anchored to this accidental) so \gre@dimen@lastglyphwidth
  % is already correct when that fires; preserve the existing rule that
  % accidentals inside a custos do not update it.
...
\def\GreBracket#1#2#3#4{%
  ...
  % Measure the bracket glyph and record its width before \gre@newglyphcommon
  % (which may typeset pending above-lines text anchored to it) so
  % \gre@dimen@lastglyphwidth is already correct when that fires.
