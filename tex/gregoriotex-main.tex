...
\def\GreScoreLyricLines#1{%
  ...
  % lyricstackseparation per extra level (see \GreWriteLyricLine). That
  % extra depth needs corresponding extra headroom in the interline glue
  % above the *next* system, or TeX's own baselineskip/lineskiplimit
  % logic can silently substitute \lineskip (a few points) for
  % \baselineskip (tens of points), collapsing that one gap. This is
  % handled per system transition in Lua (see adjust_glue and its
  % prev_stack_extra/forgive_depth parameters in gregoriotex.lua), which
  % reserves the extra room only where a system actually used stacking,
  % rather than widening \baselineskip for the whole score.
}%
