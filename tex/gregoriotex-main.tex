...
% #3: per-voice right-balance dimen register to set
...
\def\gre@nabc@prepare#1#2#3{%
  ...
  % Set per-voice right-balance to the NABC box width so the per-voice
  % kern decision in \gre@newglyphcommon knows which voice overflowed.
  % The combined rightbalance is recomputed as MAX(@i, @ii) in phase 2.
...
\def\GreSetNabcAboveLines#1{%
  \gdef\gre@nabc@pre@voice@i{%
    \gre@nabc@prepare{#1}\gre@box@nabc@voice@i\gre@dimen@nabc@rightbalance@i%
    \gdef\gre@nabc@pre@voice@i{}%
  }%
...
\def\GreSetNabcBelowLines#1{%
  \gdef\gre@nabc@pre@voice@ii{%
    \gre@nabc@prepare{#1}\gre@box@nabc@voice@ii\gre@dimen@nabc@rightbalance@ii%
    \gdef\gre@nabc@pre@voice@ii{}%
  }%
...
\def\GreEndOfElement#1#2#3{%
  ...
  \fi%
  \gre@nabc@reduce@rightbalance{\gre@skip@temp@four}%
  \ifgre@unbreakableendofelement %
    \GreNoBreak %
  \fi %
\relax%
