...
% the number of lyric lines of the score (1 = no stacked lyrics)
\newcount\gre@count@lyriclines%
\gre@count@lyriclines=1\relax%

\def\GreScoreLyricLines#1{%
  \global\gre@count@lyriclines=#1\relax%
  % Stacked lyric lines (level 2+) extend below the main baseline by
  % lyricstackseparation per extra level (see \GreWriteLyricLine). Systems
  % without that extra content report a shallower box, so a fixed
  % \baselineskip leaves no headroom for it: whenever a nearby system's own
  % height/depth (e.g. from a custos or a tall note) additionally eats into
  % that margin, the interline glue can go negative and TeX silently
  % substitutes \lineskip (a few points) for \baselineskip (tens of points),
  % collapsing the gap between systems instead of spacing them uniformly.
  % Widening the score's \baselineskip by the extra depth stacking can use
  % keeps that margin positive throughout the score, so this fallback is not
  % needed on its account.
  \ifnum#1>1\relax%
    \gre@dimen@temp@one=\gre@space@dimen@lyricstackseparation\relax%
    \multiply\gre@dimen@temp@one by \numexpr#1-1\relax%
    \advance\baselineskip by \gre@dimen@temp@one\relax%
  \fi%
}%

\newdimen\gre@dimen@lyriclinecenter%
\newdimen\gre@dimen@lyriclineraise%

% Writes one additional lyric line (level 2+) of a stacked syllable.  This is
% expanded inside the eighth argument of \GreSyllable, where the current
% point is the left edge of the level-1 syllable text and
% \gre@dimen@textaligncenter still holds the level-1 alignment center, so
% that kerning by the difference of the alignment centers puts this line's
% center exactly under the level-1 center.
...
\def\GreWriteLyricLine#1#2#3#4#5#6#7#8{%
  \ifgre@showlyrics%
    \ifcase#3\relax% no forced center: honor the centering mode
      \ifcase\gre@lyriccentering% 0 - syllable centering
        \def\gre@lyricline@firstpart{}%
        \def\gre@lyricline@middlepart{#4#5#6}%
        \def\gre@lyricline@endpart{}%
      \or% 1 - vowel centering
        \def\gre@lyricline@firstpart{#4}%
        \def\gre@lyricline@middlepart{#5}%
        \def\gre@lyricline@endpart{#6}%
      \or% 2 - first-letter centering
        \def\gre@lyricline@firstpart{}%
        \def\gre@lyricline@middlepart{#7}%
        \def\gre@lyricline@endpart{#8}%
      \fi%
    \else% forced center
      \def\gre@lyricline@firstpart{#4}%
      \def\gre@lyricline@middlepart{#5}%
    ...
    \gre@widthof{\gre@lyricline@firstpart\gre@lyricline@middlepart}%
    \gre@dimen@lyriclinecenter=\gre@dimen@temp@three%
    \gre@widthof{\gre@lyricline@middlepart}%
    \divide\gre@dimen@temp@three by 2\relax%
    \advance\gre@dimen@lyriclinecenter by -\gre@dimen@temp@three%
    % each level sits lyricstackseparation below the previous one
    \gre@dimen@lyriclineraise=\gre@space@dimen@lyricstackseparation\relax%
    \multiply\gre@dimen@lyriclineraise by \numexpr#1-1\relax%
    \gre@dimen@lyriclineraise=\dimexpr\gre@space@dimen@spacebeneathtext-\gre@dimen@lyriclineraise\relax%
    \directlua{gregoriotex.set_lyric_line_dash(#1, #2)}%
    \raise\gre@dimen@lyriclineraise%
    \hbox attr \gre@attrid@part=\numexpr9+#1\relax to 0pt{%
      \kern\dimexpr\gre@dimen@textaligncenter-\gre@dimen@lyriclinecenter\relax%
      \hbox{\gre@lyricline@firstpart\gre@lyricline@middlepart\gre@lyricline@endpart}%
      \hss}%
  \fi%
}%
