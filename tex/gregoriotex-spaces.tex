...
%% @desc Macro computing minimum distances between notes/text of this syllable and next
...
\def\gre@calculate@eolshift#1{%
  ...
  % which is added afterwards in Lua and we need to make some room for
  ...
  \ifgre@possibleluahyphenafterthissyllable %
    ...
    \gre@debugmsg{eolshift}{width of the potential hyphen: \the\gre@dimen@temp@three}%
  \fi %
  ...
  \gre@debugmsg{eolshift}{hyphen-enddifference: \the\gre@skip@temp@two}%
  ...
  \ifdim#1<0pt
    ...
    \ifdim\gre@skip@temp@two>\gre@skip@temp@three%
      \gre@debugmsg{eolshift}{text longer than notes, hyphen past custos}%
      \global\gre@dimen@eolshift = \glueexpr(\gre@skip@temp@three-\gre@dimen@temp@three)\relax %
    \else%
      \gre@debugmsg{eolshift}{text longer than notes, hyphen not past custos}%
      \global\gre@dimen@eolshift = \glueexpr(\gre@skip@temp@two-\gre@dimen@temp@three)\relax
    \fi%
  \else % enddifference > 0
    \ifdim\gre@skip@temp@two>\gre@skip@temp@three
      \gre@debugmsg{eolshift}{text shorter than notes, hyphen past custos}%
      \global\gre@dimen@eolshift = \glueexpr(\gre@skip@temp@three-\gre@skip@temp@two)\relax
    \else
      \gre@debugmsg{eolshift}{text shorter than notes, hyphen not past custos}%
      \global\gre@dimen@eolshift=0pt
    \fi
  \fi %
...
\newcount\gre@space@count@additionalbottomspacenabcthreshold\relax%
\gre@space@count@additionalbottomspacenabcthreshold=2
...
%% #1: The first part of the syllable
%% #2: The middle part of the syllable
%% #3: 0 if it's the current syllable, 1 if it's the following one
...
% Computes the "alignment center" of a piece of text -- the horizontal
% distance from its left edge to the middle of its middle part -- and
% stores it (globally) in dimen register #3. Shared by
% \gre@calculate@textaligncenter (level 1, which wraps #1/#2 in
% \gre@fixedtextformat/\gre@fixednexttextformat before calling this) and
% \GreWriteStackedLyric's per-level centering (levels 2+, which wrap #1/#2
% in \gre@fixedtextformat the same way).
% #1: the first+middle text combined
% #2: the middle text alone
% #3: the dimen register to store the result into
\def\gre@measurecenter#1#2#3{%
  \gre@widthof{#1}%
  \global#3=\the\gre@dimen@temp@three\relax%
  \gre@widthof{#2}%
  \divide\gre@dimen@temp@three by 2 %
  \global\advance#3 by -\the\gre@dimen@temp@three\relax%
}%

\def\gre@calculate@textaligncenter#1#2#3{%
  \gre@trace{gre@calculate@textaligncenter{#1}{#2}{#3}}%
  \ifnum#3=0\relax
    \gre@measurecenter{\gre@fixedtextformat{#1#2}}{\gre@fixedtextformat{#2}}{\gre@dimen@textaligncenter}%
  \else %
    \gre@measurecenter{\gre@fixednexttextformat{#1#2}}{\gre@fixednexttextformat{#2}}{\gre@dimen@textaligncenter}%
  \fi %
  \relax%
  \gre@trace@end%
}%
...
%% 5 : alteration type (see \gre@alteration)
\def\gre@calculate@nextbegindifference#1#2#3#4#5{%
  ...
  \else %
    ...
    % if the next syllable itself has stacked lyric lines wider than its own
    % level 1, widen the lookahead too (mirrors \gre@dimen@extraleftoverhang
    % for the current syllable, tex/gregoriotex-syllable.tex)
    \gre@measurenextoverhangs{\gre@dimen@textaligncenter}%
    \gre@dimen@temp@four=\gre@dimen@notesaligncenter\relax%
    \global\gre@skip@nextbegindifference=-\gre@dimen@textaligncenter\relax%
    \global\advance\gre@skip@nextbegindifference by \gre@dimen@nextextraleftoverhang\relax%
    ...
    \gre@calculate@nextnotesaligncenter{#4}{#5}% idem
...
\def\gre@calculate@barposition#1#2{%
  ...
  %we also need to correct minNotesDistance and minTextDistance because they depend on enddifference
  \gre@debugmsg{barspacing}{Correcting minimum text and notes distances}%
...
\def\gre@clearsyllable#1{%
  ...
  \ifdim\gre@dimen@temp@one > 0pt\relax%
    \ifdim\gre@dimen@begindifference < 0pt\relax%
      \ifdim\gre@dimen@temp@one > -\gre@dimen@begindifference\relax%
        \gre@debugmsg{clear}{Case 1: kern \the\dimexpr-\gre@dimen@begindifference\relax}%
        \kern -\gre@dimen@begindifference\relax%
      \else%
        \gre@debugmsg{clear}{Case 2: kern \the\gre@dimen@temp@one}%
  ...
  \else%
    \ifdim\gre@dimen@begindifference > 0pt\relax%
      \ifdim-\gre@dimen@temp@one > \gre@dimen@begindifference\relax%
        \gre@debugmsg{clear}{Case 3: kern \the\gre@dimen@begindifference}%
        \kern \gre@dimen@begindifference\relax%
      \else%
        \gre@debugmsg{clear}{Case 4: kern \the\dimexpr-\gre@dimen@temp@one\relax}%
        \kern -\gre@dimen@temp@one\relax%
      \fi%
    \else%
      \gre@debugmsg{clear}{Syllable already clear}%
...
% horizontal offset used to center/right-align above-lines text relative to
% its anchor glyph (computed in \gre@typesettextabovelines)
\newdimen\gre@dimen@abovelinestextoffset\relax%
...
\def\grechangenextscorelinedim#1#2#3#4{%
  \IfStrEqCase{#2}{{abovelinestextheight}{}{abovelinestextraise}{}{abovelinesnabcheight}{}{abovelinesnabcraise}{}{spaceabovelines}{}{belowlinesnabcheight}{}{spacelinestext}{}{noteadditionalspacelinestext}{}{translationheight}{}{lyricstackseparation}{}{spacebeneathtext}{}}[%
    \gre@error{Unrecognized option "#2" for \protect\grechangenextscorelinedim\MessageBreak Possible options are: 'spaceabovelines', 'abovelinestextheight', 'abovelinestextraise', 'abovelinesnabcheight', 'abovelinesnabcraise', 'belowlinesnabcheight', 'spacelinestext', 'noteadditionalspacelinestext', 'translationheight', 'lyricstackseparation', 'spacebeneathtext'}%
  ]%
  {\grechangedim{#2}{#3}{#4}%
...
\def\grechangenextscorelinecount#1#2#3{%
  \IfStrEqCase{#2}{{additionaltopspacethreshold}{}{additionaltopspacealtthreshold}{}{additionaltopspacenabcthreshold}{}{noteadditionalspacelinestextthreshold}{}{additionalbottomspacenabcthreshold}{}}[%
    \gre@error{Unrecognized option "#2" for \protect\grechangenextscorelinecount\MessageBreak Possible options are: 'additionaltopspacethreshold', 'additionaltopspacealtthreshold', 'additionaltopspacenabcthreshold', 'noteadditionalspacelinestextthreshold', 'additionalbottomspacenabcthreshold'}%
  ]%
  \directlua{
