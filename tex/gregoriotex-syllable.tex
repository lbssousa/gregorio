...
\def\gre@newglyphcommon{%
  ...
  % Inter-glyph NABC right padding (phase 1 of 2): save the previous
  % glyph's remaining right-balance and reset to 0 so the NABC pre-phase
  % can set a fresh value if this glyph carries new NABC content.
  \ifgre@nabc@interglyphpadding%
    \gre@dimen@nabc@prevbalance=\gre@dimen@nabc@rightbalance\relax%
    \gre@dimen@nabc@prevbalance@i=\gre@dimen@nabc@rightbalance@i\relax%
    \gre@dimen@nabc@prevbalance@ii=\gre@dimen@nabc@rightbalance@ii\relax%
    \global\gre@dimen@nabc@rightbalance=0pt\relax%
    \global\gre@dimen@nabc@rightbalance@i=0pt\relax%
    \global\gre@dimen@nabc@rightbalance@ii=0pt\relax%
  \fi%
  ...
  \fi%
  % Inter-glyph NABC right padding (phase 2 of 2): emit the compensating
  % kern only when BOTH (a) the previous glyph's NABC overflowed AND
  % (b) at least one voice that overflowed also has new NABC content in
  % the current glyph (checked per-voice via rightbalance@i/@ii).
  % Voices that overflowed but have empty next snippets do NOT contribute
  % to the kern — this prevents spurious separation when e.g. voice i
  % has ||cl- (empty voice i) after a wide tssun1 neume.
  % Voices without new content have their rightbalance restored so the
  % overflow continues to be tracked and reduced by subsequent advances.
  \ifgre@nabc@interglyphpadding%
    \ifdim\gre@dimen@nabc@prevbalance>0pt\relax%
      \gre@dimen@nabc@prevbalance=0pt\relax% reuse as kern accumulator
      % Voice i
      \ifdim\gre@dimen@nabc@prevbalance@i>0pt\relax%
        \ifdim\gre@dimen@nabc@rightbalance@i>0pt\relax%
          \ifdim\gre@dimen@nabc@prevbalance@i>\gre@dimen@nabc@prevbalance\relax%
            \gre@dimen@nabc@prevbalance=\gre@dimen@nabc@prevbalance@i\relax%
          \fi%
        \else%
          % Voice i overflowed but next snippet is empty: restore for tracking.
          \global\gre@dimen@nabc@rightbalance@i=\gre@dimen@nabc@prevbalance@i\relax%
        \fi%
      \fi%
      % Voice ii
      \ifdim\gre@dimen@nabc@prevbalance@ii>0pt\relax%
        \ifdim\gre@dimen@nabc@rightbalance@ii>0pt\relax%
          \ifdim\gre@dimen@nabc@prevbalance@ii>\gre@dimen@nabc@prevbalance\relax%
            \gre@dimen@nabc@prevbalance=\gre@dimen@nabc@prevbalance@ii\relax%
          \fi%
        \else%
          % Voice ii overflowed but next snippet is empty: restore for tracking.
          \global\gre@dimen@nabc@rightbalance@ii=\gre@dimen@nabc@prevbalance@ii\relax%
        \fi%
      \fi%
      % Emit kern if any voice needs it; rightbalances already restored above.
      % NOTE: when both voices overflow simultaneously but with alternating
      % content, the emitted kern is not subtracted from restored
      % rightbalance@v registers, so subsequent kerns may be slightly
      % overestimated (conservative: never causes overlap).
      \ifdim\gre@dimen@nabc@prevbalance>0pt\relax%
        \kern\gre@dimen@nabc@prevbalance\relax%
      \fi%
    \fi%
    % Recompute combined rightbalance = MAX(@i, @ii) unconditionally,
    % covering both the first glyph (where prevbalance=0 and per-voice
    % registers are freshly set by \gre@nabc@prepare) and subsequent
    % glyphs (where per-voice restores may have updated @i/@ii).
    \global\gre@dimen@nabc@rightbalance=\gre@dimen@nabc@rightbalance@i\relax%
    \ifdim\gre@dimen@nabc@rightbalance@ii>\gre@dimen@nabc@rightbalance\relax%
      \global\gre@dimen@nabc@rightbalance=\gre@dimen@nabc@rightbalance@ii\relax%
    \fi%
  \fi%
...
\def\GreGlyph#1#2#3#4#5#6#7{%
  ...
  \fi
  \gre@nabc@reduce@rightbalance{\gre@dimen@lastglyphwidth}%
...
\def\gre@syllablenotes#1{%
  ...
  \global\gre@dimen@nabc@rightbalance@i=0pt\relax %
  \global\gre@dimen@nabc@rightbalance@ii=0pt\relax %
  % prevbalance registers are scratch locals: reset without \global.
  \gre@dimen@nabc@prevbalance=0pt\relax %
  \gre@dimen@nabc@prevbalance@i=0pt\relax %
  \gre@dimen@nabc@prevbalance@ii=0pt\relax %
  ...
  \global\gre@firstglyphtrue%
  % Reset rightbalance and prevbalance after boxing so the real pass starts clean.
  \global\gre@dimen@nabc@rightbalance=0pt\relax %
  \global\gre@dimen@nabc@rightbalance@i=0pt\relax %
  \global\gre@dimen@nabc@rightbalance@ii=0pt\relax %
  \gre@dimen@nabc@prevbalance=0pt\relax %
