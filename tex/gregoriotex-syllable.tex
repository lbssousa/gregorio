...
\def\gre@newglyphcommon{%
  ...
  % (b) the current glyph carries new NABC content (rightbalance > 0
  % after the pre-phase).  Condition (b) avoids spurious padding through
  % / glyph breaks and @ forced neume breaks (which generate
  % \GreEndOfElement in the .gtex output), where the NABC spans the
  % whole element and no new prepare call runs.
  % When no new NABC is prepared, the balance is restored so it continues
  % to be tracked and reduced by subsequent glyph widths and spacings.
  \ifgre@nabc@interglyphpadding%
    \ifdim\gre@dimen@nabc@prevbalance>0pt\relax%
      \ifdim\gre@dimen@nabc@rightbalance>0pt\relax%
        % New NABC was prepared: emit compensating kern from previous overflow.
        \kern\gre@dimen@nabc@prevbalance\relax%
      \else%
        % No new NABC for this glyph (sub-glyph of same element):
        % restore the balance so it continues to be tracked and reduced
        % by subsequent glyph widths and spacings.
        \global\gre@dimen@nabc@rightbalance=\gre@dimen@nabc@prevbalance\relax%
      \fi%
    \fi%
  \fi%
