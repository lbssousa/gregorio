...
\def\GreGlyph#1#2#3#4#5#6#7{%
  % Create box containing the new glyph, and record its width, before
  % \gre@newglyphcommon so that \gre@dimen@lastglyphwidth (used to
  % center/right-align any pending above-lines text against this glyph) is
  % correct when \gre@currenttextabovelines fires from within it.
...
% #5 (\GreGlyphHeights plus ledger lines, see \GreGlyph) is still run, in a
% zero-width phantom, so that collision avoidance keeps seeing this glyph's
% real pitch extrema.  The box is load-bearing: \GreGlyphHeights only sets
% attributes, which are picked up by the next node created, so dropping the
% box would leave the extrema unreported.
...
% is suppressed.  \GreGlyphHeights is deliberately left alive here so that
% \gre@nabconlyglyph can still report pitch extrema for collision avoidance.
...
% The \ifcase\gre@lyriccentering 3-way switch (syllable/vowel/first-letter
% centering), shared by \GreSetThisSyllable, \GreSetNextSyllable, and
% \GreWriteStackedLyric's per-level centering (tex/gregoriotex-main.tex).
% Used only for the "not forced" case; each caller handles its own
% forced-center case, since the trigger differs (a global \if for level 1,
% an explicit arg for stacked lines).
% #1: pre letters  #2: middle letters  #3: end letters
% #4: the very first letter  #5: all letters after the first
% #6,#7,#8: control sequences to \gdef to the first/middle/end parts
\def\gre@selectcenteringparts#1#2#3#4#5#6#7#8{%
  \ifcase\gre@lyriccentering% 0 - syllable centering
    \gdef#6{}%
    \gdef#7{#1#2#3}%
    \gdef#8{}%
...
\def\GreSetThisSyllable#1#2#3#4#5{%
  ...
  \else%
    \gre@selectcenteringparts{#1}{#2}{#3}{#4}{#5}%
      {\gre@firstsyllablepart}{\gre@middlesyllablepart}{\gre@endsyllablepart}%
  \fi%
  \gre@forcedcenteringfalse%
}%
...
\def\GreSetNextSyllable#1#2#3#4#5{%
  ...
  \else%
    \gre@selectcenteringparts{#1}{#2}{#3}{#4}{#5}%
      {\gre@nextfirstsyllablepart}{\gre@nextmiddlesyllablepart}{\gre@nextendsyllablepart}%
  \fi%
  \gre@nextforcedcenteringfalse%
}%
...
\def\GreSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \gre@count@stackedlyriclevels=1\relax% reset before #1 may stash new levels
  \gre@count@nextstackedlyriclevels=1\relax% reset before #5 may stash new levels
  ...
  \gre@fixedtextformat{%
  \directlua{gregoriotex.save_syllable_info('note')}%
  }%
  ...
  % A stacked lyric line (level 2+) wider than level 1 needs extra
  % horizontal clearance around the shared alignment point; 0pt when there
  % is no stacking or level 1 is already the widest.
  \gre@measureoverhangs{\gre@firstsyllablepart}{\gre@middlesyllablepart}{\gre@endsyllablepart}%
  ...
  % (stays based on level 1 alone -- extraleftoverhang must not shift text
  % off the notes, so it's a separate \kern below instead)
  \gre@dimen@begindifference=\dimexpr(\gre@dimen@notesaligncenter - \gre@dimen@textaligncenter)\relax %
  % Reserve a wider stacked level's extra room by shifting the whole
  % syllable (notes included) right, so its left edge lands where level 1's
  % own edge would have. Before the begin-of-line shift below so it is
  % likewise discarded there, like \gre@dimen@bolshift.
  \ifdim\gre@dimen@extraleftoverhang>0pt\relax
    \kern\gre@dimen@extraleftoverhang\relax
  \fi
  ...
  \gre@calculate@nextbegindifference{\gre@evaluatenextsyllable{\gre@nextfirstsyllablepart}}{\gre@evaluatenextsyllable{\gre@nextmiddlesyllablepart}}{\gre@evaluatenextsyllable{\gre@nextendsyllablepart}}{\gre@nextalignment}{\gre@nextalteration}%
  ...
  \ifgre@showlyrics%
    \setbox\gre@box@syllabletext=\hbox{%
      ...
      \gre@fixedtextformat{\gre@pointandclick{\gre@firstsyllablepart\gre@middlesyllablepart\gre@endsyllablepart}{#6}}%
      \IfSubStr{\gre@debug}{,notespacing,}%
        % when debugging we add a zero-width line to mark the syllable bound
        {\hbox to 0pt{\rule{0.4pt}{12pt}\hss}}%
        {}% do nothing if not debugging
  ...
  \else%
    \ifnum\gre@lyrics@phantomwrapper>0\relax
      \setbox\gre@box@syllabletext=\hbox{\gre@applyphantomwrapper{\gre@lyrics@phantomwrapper}{\gre@fixedtextformat{\gre@pointandclick{\gre@firstsyllablepart\gre@middlesyllablepart\gre@endsyllablepart}{#6}}}}%
    \else
      \setbox\gre@box@syllabletext=\hbox{}%
    \fi
  \fi%
  \unsetattribute{\gre@attr@part}%
  % fold extrarightoverhang into the text-width argument, so enddifference
  % reserves room for whichever level extends furthest right
  \gre@calculate@enddifference{\wd\gre@box@syllablenotes}{\dimexpr\wd\gre@box@syllabletext+\gre@dimen@extrarightoverhang\relax}{\gre@dimen@textaligncenter}{\gre@dimen@notesaligncenter}{1}%
  ...
  \ifcase#4
    ...
    \ifgre@showhyphenafterthissyllable
      \directlua{gregoriotex.current_syllable().dash=5}% needs forced hyphen
    \else
      \directlua{gregoriotex.current_syllable().dash=1}% maybe needs hyphen
    \fi
  \else
    ...
    \ifgre@showhyphenafterthissyllable
      \directlua{gregoriotex.current_syllable().dash=5}% needs forced hyphen
    \else
      \directlua{gregoriotex.current_syllable().dash=3}% end of word
    \fi
  \fi
  \ifgre@textcleared%
    % Insert a zero kern that will be adjusted in the Lua function syllable_clearing.
    {\gre@attr@skip@type=5 \kern0pt}%
  \fi%
  ...
  {\gre@attr@skip@type=2
  \ifdim\gre@dimen@begindifference > 0 pt\relax%
    \gre@skip@temp@one = \gre@dimen@begindifference\relax%
    \kern\gre@skip@temp@one %
  \fi}%
  #8\relax %
  % Typeset any stacked lyric lines (level 2+) stashed by
  % \GreSetStackedSyllable inside #1, now that \gre@dimen@textaligncenter
  % is free to read (\gre@calculate@enddifference above has already
  % consumed it for level 1's note-relative positioning). This must not
  % move earlier in \GreSyllable, or it would corrupt that positioning.
  ...
  \fi %
  {\gre@attr@skip@type=3
  \kern\dimexpr-\wd\gre@box@syllabletext-\gre@dimen@begindifference}%
  \gre@attr@part=10
  ...
  \ifgre@shownotes%
    \IfSubStr{\gre@debug}{,notespacing,}%
      ...
      {}% do nothing if not debugging
    #9% we do that instead of \unhbox\gre@syllablenotes, because it would not set the \localrightbox
...
\def\gre@syllablefinalskip{%
  % Save the computed minNotesDistance and minTextDistance
  \directlua{gregoriotex.save_min_distances()}%
  % Emit a zero-width skip, which will be resized in the Lua pre-linebreak filter.
...
\def\GreBarSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \global\advance\gre@attr@syllable@id by 1\relax %
  \gre@fixedtextformat{%
  \directlua{gregoriotex.save_syllable_info('bar')}%
  }%
  \gre@beginningofscorefalse
  \gre@possibleluahyphenafterthissyllablefalse %
  \gre@showhyphenafterthissyllablefalse %
  ...
  \directlua{gregoriotex.current_syllable().dash=4}%
  \gre@attr@part=4
  \ifgre@showlyrics%
    \setbox\gre@box@syllabletext=\hbox{%
  ...
  \ifgre@newbarspacing%
    ...
    \gre@hskip\glueexpr(\gre@skip@bar@allocation/2% right from end of previous notes to nominal middle of bar line
      ...
      +\gre@space@skip@bar@rubber)}% the rubber component
    \GreNoBreak %
    ...
    \kern\dimexpr(\gre@dimen@enddifference% move from end of text to end of bar line
      -\wd\gre@box@syllablenotes)}% back up from end of bar line to beginning
    \GreNoBreak%
    \gre@attr@part=10
    \ifgre@shownotes%
      \IfSubStr{\gre@debug}{,barspacing,}%
  ...
  \else%
    ...
    \ifdim\wd\gre@box@syllabletext = 0 pt\relax %
      ...
      \fi %
      \gre@attr@part=10
      % In case there are no notes, ensure that there is at least one
      % box with the attribute.
      \ifdim\wd\gre@box@syllablenotes=0pt
        \hbox{}%
      \fi
      \ifgre@shownotes%
        #9\relax %
...
\def\gre@setfirstsyllabletext#1#2#3#4#5#6{%
  ...
  \def\gre@opening@syllabletext{%
    % Restore the stacked-lyric-level count \GreScoreOpening saved before
    % \GreSyllable reset it to 1 -- see \gre@count@openingstackedlyriclevels
    % (gregoriotex-main.tex) for why this is needed.
    \gre@count@stackedlyriclevels=\gre@count@openingstackedlyriclevels\relax%
...
\def\GreScoreOpening#1#2#3#4#5{%
  ...
  % #5 may have called \GreSetStackedSyllable for the opening syllable's
  % stacked lyric lines; save the count before #4 (\GreSyllable) resets it
  % -- see \gre@count@openingstackedlyriclevels (gregoriotex-main.tex).
  \gre@count@openingstackedlyriclevels=\gre@count@stackedlyriclevels\relax%
